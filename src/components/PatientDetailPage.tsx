...
function PatientHistoryCard({ patient }: { patient: CardioPatient }) {
  ...
  const rows: [string, string | undefined][] = [
    ['ความดันสูงมานาน', p?.htDurationYears != null ? `${p.htDurationYears} ปี` : undefined],
    ...
    ['สูบบุหรี่', p?.smokingStatus ? `${SMOKING_TH[p.smokingStatus]}${p.smokingYears ? ` (${p.smokingYears} ปี)` : ''}` : undefined],
    ...
    return (
      <div style={{ background: 'rgba(59,130,246,.04)', border: '1.5px solid rgba(59,130,246,.25)', borderRadius: 'var(--r)', padding: '14px 16px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {htYears && <span style={{ fontSize: 12, padding: '3px 10px', borderRadius: 20, background: 'var(--bg3)', border: '1px solid var(--border)', fontFamily: 'var(--mono)' }}>ความดันมา {htYears} ปี</span>}
  ...
  return (
    <div style={{ background: 'rgba(59,130,246,.04)', border: '1.5px solid rgba(59,130,246,.2)', borderRadius: 'var(--r)', padding: '18px 20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 14 }}>
        <div>
          {lbl('ความดันสูงมานานแค่ไหน')}
          <input type="number" value={htYears} onChange={e => setHtYears(e.target.value)} style={inputStyle()} />
        </div>
        <div>
          {lbl('ส่วนสูง (ซม.)')}
          <input type="number" value={heightCm} onChange={e => setHeightCm(e.target.value)} placeholder="ซม." style={inputStyle()} />
        </div>
        <div>
          {lbl('เส้นรอบเอว (ซม.)')}
          <input type="number" value={waistCm} onChange={e => setWaistCm(e.target.value)} placeholder="ซม." style={inputStyle()} />
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 14 }}>
        ...
        {smoking !== 'never' && (
          <div>
            {lbl('สูบมากี่ปี')}
            <input type="number" value={smokingYears} onChange={e => setSmokingYears(e.target.value)} placeholder="ปี" style={inputStyle()} />
          </div>
        )}
        <div>
          {lbl('ดื่มแอลกอฮอล์')}
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 14 }}>
            ...
            <div>
              ...
              <input type="number" value={dmDiagYear} onChange={e => setDmDiagYear(e.target.value)} placeholder="เช่น 2560" style={inputStyle()} />
            </div>
            <div>
              {lbl('เป็นมากี่ปี')}
              <input type="number" value={dmYears} onChange={e => setDmYears(e.target.value)} placeholder="ปี" style={inputStyle()} />
            </div>
          </div>
        </>
      )}
      ...
      {hasCKD && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 10, marginBottom: 14 }}>
            <div>
              {lbl('เป็นมากี่ปี')}
              <input type="number" value={ckdYears} onChange={e => setCkdYears(e.target.value)} placeholder="ปี" style={inputStyle()} />
            </div>
          </div>
        </>
      )}
            ...
            {g.keys.map(k => {
              ...
              return (
                <div key={k}>
                  ...
                  <input type="number" value={vals[k] ?? ''} placeholder={f.ph || '—'}
